...
function CryptoForm({ initial, onClose }: { initial?: CryptoInvestment; onClose:()=>void }) {
  ...
  const submit = (data: z.infer<typeof cryptoSchema>) => {
    if (initial) update({ ...initial, type:'crypto' as const, coinName:data.coinName, symbol:data.symbol, quantity:data.quantity, purchasePrice:data.purchasePrice, currency:data.currency, purchaseDate:data.purchaseDate }, uid)
    else add({ id:uuid(), type:'crypto' as const, createdAt:new Date().toISOString(), coinName:data.coinName, symbol:data.symbol, quantity:data.quantity, purchasePrice:data.purchasePrice, currency:data.currency, purchaseDate:data.purchaseDate }, uid)
    onClose()
  }
  return (
...
function StakingForm({ initial, onClose }: { initial?: StakingInvestment; onClose:()=>void }) {
  ...
  const submit = (data: z.infer<typeof stakingSchema>) => {
    if (initial) update({ ...initial, type:'staking' as const, name:data.name, principal:data.principal, currency:data.currency, apr:data.apr, startDate:data.startDate, endDate:data.endDate }, uid)
    else add({ id:uuid(), type:'staking' as const, createdAt:new Date().toISOString(), name:data.name, principal:data.principal, currency:data.currency, apr:data.apr, startDate:data.startDate, endDate:data.endDate }, uid)
    onClose()
  }
  return (
    <form onSubmit={handleSubmit(submit)} style={{ display:'flex',flexDirection:'column',gap:14 }}>
...
function FixedForm({ initial, onClose }: { initial?: FixedInvestment; onClose:()=>void }) {
  ...
  const submit = (data: z.infer<typeof fixedSchema>) => {
    if (initial) update({ ...initial, type:'fixed' as const, name:data.name, principal:data.principal, currency:data.currency, interestRate:data.interestRate, startDate:data.startDate, endDate:data.endDate }, uid)
    else add({ id:uuid(), type:'fixed' as const, createdAt:new Date().toISOString(), name:data.name, principal:data.principal, currency:data.currency, interestRate:data.interestRate, startDate:data.startDate, endDate:data.endDate }, uid)
    onClose()
  }
  return (
    <form onSubmit={handleSubmit(submit)} style={{ display:'flex',flexDirection:'column',gap:14 }}>
...
export default function InvestmentsPage() {
  const uid = useUserId()
  const { investments, remove, load, loaded } = useInvestmentStore()
