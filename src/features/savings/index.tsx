...
function SavingsForm({ initial, onClose }: { initial?: SavingsEntry; onClose: () => void }) {
  ...
  const submit = (data: FormData) => {
    ...
    else add({ id:uuid(), createdAt:new Date().toISOString(), amount:data.amount, currency:data.currency, note:data.note, date:data.date }, uid)
    onClose()
  }
  return (
...
export default function SavingsPage() {
  const uid = useUserId()
  const { entries, remove, load, loaded } = useSavingsStore()
  ...
  const { settings, load: loadSett, loaded: settLoaded } = useSettingsStore()
