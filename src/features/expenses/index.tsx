...
function ExpenseForm({ initial, onClose }: { initial?: Expense; onClose: () => void }) {
  ...
  const submit = (data: FormData) => {
    ...
    else add({ id:uuid(), createdAt:new Date().toISOString(), amount:data.amount, currency:data.currency, category:data.category, note:data.note, date:data.date }, uid)
    onClose()
  }
  return (
...
export default function ExpensesPage() {
  const uid = useUserId()
  const { expenses, remove, load, loaded } = useExpenseStore()
