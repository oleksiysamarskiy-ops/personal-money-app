...
function DebtFormComp({ initial, defaultDir, onClose }: { initial?: Debt; defaultDir: 'i_owe'|'owed_to_me'; onClose:()=>void }) {
  ...
  const submit = (data: DebtForm) => {
    ...
    else add({ id:uuid(), createdAt:new Date().toISOString(), payments:[], direction:data.direction, name:data.name, amount:data.amount, currency:data.currency, note:data.note, dueDate:data.dueDate }, uid)
    onClose()
  }
...
function PaymentFormComp({ debt, onClose }: { debt: Debt; onClose:()=>void }) {
  ...
  const submit = (data: PaymentForm) => {
    addPayment(debt.id, { id:uuid(), amount:data.amount, date:data.date, note:data.note }, uid)
    onClose()
  }
  return (
    <form onSubmit={handleSubmit(submit)} style={{ display:'flex',flexDirection:'column',gap:14 }}>
...
export default function DebtsPage() {
  const uid = useUserId()
  const { debts, remove, load, loaded } = useDebtStore()
